...
def filepathstring(filepath: str):
...
if __name__ == "__main__":
    for file_path in ALL_FILE_PATHS:
        ...
        stringpath=filepathstring(file_path)
        ...
        # xml, patient = load(TEST2_540_PATH)
        # stringpath = filepathstring(TEST2_540_PATH)
        # cleaned = drop_days_with_missing_glucose_data(xml, 50)
        # write_to_xml(os.path.join(CLEANED_DATA_DIR, stringpath), cleaned, int(patient['id']), patient['insulin_type'],
        #      body_weight=int(patient['weight']))
